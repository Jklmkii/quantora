...
- **🎨 Acessibilidade Abrangente em Inputs de Texto & Busca (PR #54 — Jules AI Palette):** Inclusão de prop `ariaLabel` e fallback semântico abrangente em `NumericInput.tsx`, além de atributos `aria-label` explícitos nos campos de entrada manual do Boss Battle, busca no histórico, entrada de fórmula em Bhaskara e widget do Tray, atendendo integralmente às diretrizes WCAG AA.
- **🛡️ Blindagem Global de Segurança de Navegação no Electron (`electron/main.cjs` — PR #55 — Jules AI Sentinel):** Refatoração dos interceptadores `will-navigate` e `setWindowOpenHandler` para o listener global de ciclo de vida `app.on('web-contents-created')`, garantindo cobertura automática e uniforme contra navegação arbitrária para toda e qualquer janela ou `webContents` criado na aplicação.
